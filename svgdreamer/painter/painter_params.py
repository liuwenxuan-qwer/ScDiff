...
class Painter(DiffVGState):
    ...
    def init_image(self, num_paths=0):
        ...
        intpoint = self.get_uniform_points_within_mask(num_points=num_paths)
        ...
        for i in range(num_paths_exists, num_paths):
            if self.style == 'iconography':
                path = self.get_path()
                #path = self.get_path2(intpoint[i])
    ...
    def get_path(self, intp0=None, coord=None):
        num_segments = self.num_segments
    ...
    def get_uniform_points_within_mask(self, mask_path=None, num_points=None):
        mask_path = self.imgpath
        ...
        # 计算每个网格块的宽度和高度
        block_width = (max_x - min_x + 1) // grid_size
        block_height = (max_y - min_y + 1) // grid_size
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                # 确定当前网格块的边界
                x_start = min_x + j * block_width
                x_end = min(min_x + (j + 1) * block_width, max_x + 1)
                y_start = min_y + i * block_height
                y_end = min(min_y + (i + 1) * block_height, max_y + 1)

                # 在当前网格块中找到有效点
                block_indices = [
                    index for index in mask_indices
                    if x_start <= index[1] < x_end and y_start <= index[0] < y_end
                ]

                if block_indices:
                    # 从当前网格块中随机选择一个点
                    selected_indices.append(random.choice(block_indices))

        # 如果选择的点数不足，随机补充
        while len(selected_indices) < num_points:
            selected_indices.append(random.choice(mask_indices))

        # 添加小随机偏移，转换为归一化浮点坐标
        points = []
        for int_point in selected_indices[:num_points]:
            float_x = int_point[1] + random.uniform(-0.5, 0.5)
            float_y = int_point[0] + random.uniform(-0.5, 0.5)
            points.append([
                float(float_x / mask.shape[1]),  # 宽度归一化
                float(float_y / mask.shape[0])  # 高度归一化
            ])
        return points
    
    def get_path_re(self, coord=None):
            num_segments = self.num_segments
    ...
    def get_point_within_mask(self, mask_path = None):
        mask_path = self.imgpath
        mask_image = Image.open(mask_path).convert('L')
        mask = np.array(mask_image)
        ...
        int_point = random.choice(mask_indices)

        float_x = int_point[1] + random.uniform(-0.5, 0.5)
        float_y = int_point[0] + random.uniform(-0.5, 0.5)

        return [float(float_x/mask.shape[0]), float(float_y/mask.shape[1])]    
        ...
        if len(reinit_union) > 0:
            for i, path in enumerate(self.shapes):
                if path.id in reinit_union:
                    ...
                    self.shapes[i] = self.get_path_re(coord=coord)
                    # new point
