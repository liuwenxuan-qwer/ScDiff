...
from svgdreamer.token2attn.ptp_utils2 import AttentionStore,  hook_attention_processors , prepare_bboxes, compute_cross_loss, compute_self_loss
...
class VectorizedParticleSDSPipeline(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.attention_store = AttentionStore(16)
        hook_attention_processors(self.sd_pipeline, self.attention_store)
    ...
    def boxloss(self, pipeline, attention_store, latents, prompt, indices, fg_bounding_boxes, bg_bounding_boxes, device, t):
    ...
    def variational_score_distillation(self,
                                       ...
                                       im_size: int = 512,
                                       index : list = [],
                                       fg_bounding_boxes : list = [],
                                       as_latent: bool = False):
        bz = pred_rgb.shape[0]

        # data enhancement for the input particles
        pred_rgb = self.x_augment(pred_rgb, im_size) if enhance_particle else pred_rgb
        let_rgb = self.x_augment(pred_rgb, im_size) if enhance_particle else pred_rgb
        # interp to 512x512 to be fed into vae.
        if as_latent:
            latents = F.interpolate(pred_rgb, (64, 64), mode='bilinear', align_corners=False) * 2 - 1
            #latents_let = F.interpolate(let_rgb, (64, 64), mode='bilinear', align_corners=False) * 2 - 1
        else:
            pred_rgb_ = F.interpolate(pred_rgb, (512, 512), mode='bilinear', align_corners=False)
            let_rgb_ = F.interpolate(let_rgb, (512, 512), mode='bilinear', align_corners=False)
            # encode image into latents with vae, requires grad!
            # latents = self.encode2latent(pred_rgb_)
            latent_list = [self.encode2latent(pred_rgb_[i].unsqueeze(0)) for i in range(bz)]
            latent_list_let = [self.encode2latent(let_rgb_[i].unsqueeze(0)) for i in range(bz)]
            latents = torch.cat(latent_list, dim=0)
            latents_let = torch.cat(latent_list_let, dim=0)
            alpha = 0.2  
            latents = latents * (1 - alpha )+latents_let * alpha
        ...
        boxlosses  = torch.tensor(0.)
        # for i in range(bz):
    
        #     latent = latents[i].unsqueeze(0)
        #     boxloss = self.boxloss(self.sd_pipeline, self.attention_store, latent, prompt, index,fg_bounding_boxes, bg_bounding_boxes, self.device, self.t)
        #     boxlosses = boxlosses + boxloss
        ...
        return loss_vpsd, grad.norm(), latents, self.t, boxlosses
