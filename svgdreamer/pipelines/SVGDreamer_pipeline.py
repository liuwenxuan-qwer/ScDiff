...
from svgdreamer.painter import (CompPainter, CompPainterOptimizer, xing_loss_fn, Painter, PainterOptimizer,
                                CosineWithWarmupLRLambda, VectorizedParticleSDSPipeline, DiffusionPipeline,ToneLoss,ImageToSDFLoss)
...
class SVGDreamerPipeline(ModelState):
    ...
    def __init__(self, args):
        ...
        self.mseweight = 160
        #boxloss
        self.boxlossweight = 5
        #self.mseweights = self.loss_weight_scheduler(1000, 200, 2000, 10)
        #self.mseweights = self.loss_weight_scheduler2(1500, 400, 600, 2000, 500, 5)
        self.mseweights = self.gaussian_weight_scheduler(800, 200, 2000, 800, 200)
        self.boxlossweights = self.loss_weight_scheduler2(6, 1, 2, 2000, 500, 3)
               
        ###############
        self.sdfimage_path = self.get_letter_sdfimage_path(self.args.let)
        self.let_imgs_path = self.get_letter_image_path(self.args.let)
        self.r_let_tensor = self.load_and_expand_image(self.sdfimage_path)
        self.r_let_tensor = self.r_let_tensor.to(self.device) 
        self.let_imgs = self.load_and_expand_image(self.let_imgs_path)
        self.let_imgs = self.let_imgs.to(self.device)
        ###############
        self.box = self.args.box
        self.index = self.args.boxind
    ...
    def VPSD_stage(self,
        ...
        with tqdm(initial=self.step, total=total_step, disable=not self.accelerator.is_main_process) as pbar:
            while self.step < total_step:
                ...
                L_guide, grad, latents, t_step, L_box = pipeline.variational_score_distillation(
                    raster_imgs.to(self.weight_dtype),
                    self.let_imgs.to(self.weight_dtype),
                    self.step,
                    prompt=[text_prompt],
                    ...
                    index = self.index,
                    fg_bounding_boxes = self.box
                )
                #sdf_loss_fn = ImageToSDFLoss()
                #L_let = sdf_loss_fn(self.r_let_tensor, raster_imgs)

                # 加上一个lossmseloss 用字母图像和现在渲染的图像计算
                # L_let = toneloss(r_let_tensor, raster_imgs) 
                #L_let = ImageToSDFLoss(self.r_let_tensor, raster_imgs)
                L_let = self.mse_loss(self.r_let_tensor.detach(), raster_imgs)*self.mseweight
                # L_box = L_box*self.boxlossweight
                ...
                #loss = L_guide + L_add + L_let + L_box
                
                if self.step < 100:
                    loss = L_guide*0.001 + L_add + L_box*10 + L_let*0.01
                elif self.step < 200:
                    loss = L_guide*0.1 + L_add + L_box*10 + L_let*0.1
                elif self.step < 800:
                    if self.step % 4 == 0:
                        loss = L_guide*0.001 + L_add + L_box*5 + L_let*8
                    else:
                        loss = L_guide + L_add + L_box*5 + L_let*4
                else:
                    loss = L_guide + L_add  + L_box*0.3 + L_let*0.2
    ...
    def load_and_expand_image(self, image_path, num_copies=4):
        # 读取图片
        ...
        # 增加一个维度
        image_tensor = image_tensor.unsqueeze(0)  # 在第0维增加一个维度
        # 在新增加的维度上复制指定次数
        image_tensor_expanded = image_tensor.repeat(num_copies, 1, 1, 1)
        return image_tensor_expanded
    
   
    def loss_weight_scheduler(self, max_factor, min_factor, total_steps, decay_rate):

        # 构造步数范围
        steps = np.arange(total_steps)

        # 使用指数衰减函数构造递减曲线
        weights = max_factor - (max_factor - min_factor) * (1 - np.exp(-decay_rate * steps / total_steps))

        return weights

    def loss_weight_scheduler2(self, max_value, min_value, transition_value, total_steps, transition_step, decay_rate):

        # 初始化权重数组
        weights = np.zeros(total_steps)

        # 前半部分使用指数衰减（凸函数）
        for i in range(transition_step):
            weights[i] = max_value - (max_value - transition_value) * (i / transition_step) ** decay_rate

        # 后半部分使用对数衰减（凹函数）
        for i in range(transition_step, total_steps):
            weights[i] = transition_value - (transition_value - min_value) * np.log1p(i - transition_step) / np.log1p(
                total_steps - transition_step)

        return weights
    def gaussian_weight_scheduler(self, max_value, min_value, total_steps, center_step, gradient):

        steps = np.arange(total_steps)
        sigma = gradient  
        gaussian = max_value * np.exp(-((steps - center_step) ** 2) / (2 * sigma ** 2))
        weights = min_value + (gaussian - np.min(gaussian)) / (np.max(gaussian) - np.min(gaussian)) * (
                    max_value - min_value)

        return weights
    
    def get_letter_image_path(self, letter, output_dir="output_letters"):
    ...
    def get_letter_sdfimage_path(self, letter, output_dir="output_letters"):
        if len(letter) != 1 or not letter.isalpha():
            print("错误：输入必须是单个字母。")
            return None
        letter = letter.upper()
        image_path = os.path.join(output_dir, f"sdf{letter}.png")
        if os.path.exists(image_path):
            return image_path
        else:
            print(f"图片不存在：{image_path}")
